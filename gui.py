...
class MainFrame(ttk.Frame):
    def __init__(self, container):
        ...
        self.gravity_var.trace(
            "w",
            lambda *args: self.gravity_val_label.config(
                text=f"{self.gravity_var.get():.2f}"
            ),
        )

        ttk.Label(self.control_frame, text="Air Resistance").pack(pady=2)
        self.air_resistance_var = tk.DoubleVar(value=0.1)
        self.air_resistance_slider = ttk.Scale(
            self.control_frame,
            from_=0,
            to=1,
            orient=tk.HORIZONTAL,
            variable=self.air_resistance_var,
        ...
        self.air_resistance_var.trace(
            "w",
            lambda *args: self.air_resistance_val_label.config(
                text=f"{self.air_resistance_var.get():.2f}"
            ),
        )
        ...
        self.friction_var.trace(
            "w",
            lambda *args: self.friction_val_label.config(
                text=f"{self.friction_var.get():.2f}"
            ),
        )
    ...
    def draw_body(self, body, body_id):
        ...
        return self.canvas.create_polygon(
            points, outline="white", fill="", tags=("body", f"body_{body_id}")
        )
    ...
    def update_canvas(self):
        for item in self.physics.rigid_bodies:
            ...
            if canvas_id:
                points = [
                    coord
                    for corner in body.get_corners()
        ...
        if self.debug_mode.get():
            for item in self.physics.rigid_bodies:
                ...
                v_scale = 0.5
                end_v = Vector2D(
                    pos.x + body.velocity.x * v_scale, pos.y + body.velocity.y * v_scale
                )
                ...
                f_scale = 0.1
                end_f = Vector2D(
                    pos.x + body.constant_force.x * f_scale,
                ...
                end_a = Vector2D(
                    pos.x + air_resistance.x * a_scale,
