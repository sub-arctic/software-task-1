...
class Button(ttk.Button):
    """A clickable button widget residing in a frame"""
    def __init__(self, parent, *args, **kwargs):
        super().__init__(parent, *args, **kwargs)

class Canvas(tk.Canvas):
    """A rectanglar area intended for drawing pictures or other complex layouts"""
    def __init__(self, parent, *args, **kwargs):
        super().__init__(parent, *args, **kwargs)
...
class SimulationScreen(Application):
    def __init__(self, parent, width=512, height=512):
        ...
        self.parent = parent
        self.canvas = Canvas(self, bg=Defaults.BACKGROUND, width=width, height=height)
        self.canvas.pack(fill=tk.BOTH, expand=True)

        self.start_button = Button(self, text="start", command=self.run_simulation)
    ...
    def run_simulation(self, dt=0.016):
        if not self.simulation_running:
            self.simulation_running = True
            self.step_simulation(dt)
    
    def step_simulation(self, dt=0.016):
        self.physics_engine.update(dt, self.canvas.winfo_width(), self.canvas.winfo_height())
        if self.simulation_running:
            self.update_canvas()
            self.parent.after(16, self.step_simulation)
